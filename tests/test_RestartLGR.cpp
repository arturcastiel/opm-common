...
#define BOOST_TEST_MODULE Restart_File_IO_LGR
...
data::Wells mkWells()
{
    ...
    w1.connections.push_back( {0, rc1, 30.45, 123.4, 543.21, 0.62, 0.15, 1.0e3, 1.234, 0.0, 1.23, con_filtrate, 1} );
    ...
    w2.temperature = 4.56;
    w2.control = 2;
    w2.connections.push_back( { 8, rc3, 36.22, 123.4, 256.1, 0.55, 0.0125, 314.15, 3.456, 0.0, 2.46, con_filtrate,2 } );
...
RestartValue
first_simLGR(const Setup&         setup,
             const Action::State& action_state,
             SummaryState&        st,
             UDQState&            udq_state,
             bool                 write_double)
{
    ...
    std::vector<int> num_cells(setup.grid.get_numCellLGR() + 1);
    auto lgr_labels = setup.grid.get_all_lgr_labels();
    std::vector<data::Solution> sol(num_cells.size());

    // Global Cells
    num_cells[0] = setup.grid.getNumActive();
    sol[0] = mkSolution(num_cells[0]);

    // Local Grid Refinement Cells
    size_t index = 1;
    for (const auto& label : lgr_labels) {
        num_cells[index] = setup.grid.getLGRCell(label).getTotalActiveLGR();
        sol[index] = mkSolution(num_cells[index]);
        ++index;
    }
    ...
    RestartValue restart_value(sol[0], wells, groups, {});
    eclWriter.writeTimeStep(action_state,
                            wtest_state,
                            st,
                            udq_state,
...
BOOST_AUTO_TEST_CASE(EclipseReadWriteWellStateDataLGR)
{
    ...
    test_area.copyIn("BASE_SIMLGR.DATA");
    test_area.copyIn("RESTART_SIMLGR.DATA");

    Setup base_setup("BASE_SIMLGR.DATA");
    auto st = sim_state(base_setup.schedule);
    Action::State action_state;
    UDQState udq_state(19);
    const auto state1 = first_simLGR( base_setup , action_state, st, udq_state, false );

    Setup restart_setup("RESTART_SIMLGR.DATA");
