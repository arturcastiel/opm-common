               ...
               const Schedule&              schedule)
{
    ...
    for (const auto& record : keyword) {
        ...
        const bool coords_defaulted = record.getItem(2).defaultApplied(0);
        const int  i = coords_defaulted ? 0 : record.getItem(2).get<int>(0);
        const int  j = coords_defaulted ? 0 : record.getItem(3).get<int>(0);
        const int  k = coords_defaulted ? 0 : record.getItem(4).get<int>(0);
...
bool operator<(const SummaryConfigNode& lhs, const SummaryConfigNode& rhs)
{
    ...
    case SummaryConfigNode::Category::Group:
    {
        // Ordering determined by named entity, then LGR identity.
        if (lhs.namedEntity() != rhs.namedEntity())
            return lhs.namedEntity() < rhs.namedEntity();
        const auto& ll = lhs.lgr();
        const auto& rl = rhs.lgr();
        if (ll.has_value() != rl.has_value())
            return rl.has_value(); // no-lgr sorts before lgr
        if (!ll.has_value())
            return false;          // both null → equal
        if (ll->name != rl->name)
            return ll->name < rl->name;
        return ll->ijk < rl->ijk;  // std::array has operator< (C++11)
    }
    ...
    case SummaryConfigNode::Category::Block:
    {
        // Ordering determined by numeric entity, then LGR identity.
    ...
    case SummaryConfigNode::Category::Segment:
    {
        // Ordering determined by named entity, numeric ID, then LGR identity.
