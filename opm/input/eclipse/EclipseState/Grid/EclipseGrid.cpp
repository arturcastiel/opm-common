...
=======
            if (this->lgr_label.compare(lgr_cell.PARENT_NAME()) == 0) {
>>>>>>> a557dcca8 (EGrid.hpp seems to contain a bug where get<float>(COORD) cannot catpure the global COORD in EGRIDs, EclipseGrid has been enhaced with functions to output .EGRIDS for LGR - unfinished work - devising a mechanism to define HOSTCEL)
                lgr_grid = true;
...
namespace Opm {
    EclipseGridLGR::EclipseGridLGR(const std::string& self_label, const std::string& father_label_, 
                                   ...
                                   std::array<int,3> low_fahterIJK_, std::array<int,3> up_fahterIJK_)
