...
namespace Opm {
    ...
    std::vector<double> EclipseGridLGR::generate_refined_coord(const std::vector<double>& coord_h,
                                                               const std::array<int,3>&   parent_nxyz)
    {
        auto interpolatePillar = [](const double* p1, const double* p2, const double t, double* p) {
        ...
        for (auto J = Jmin; J <= Jmax; ++J) {
            for (auto j = 0*sj; j <= sj; ++j) {
                ...
                // interpolates first pillar (I_min) in between for tj in between Jmin and Jmax
                auto* right = &coord_c[ cc.index(0, (J - Jmin)*sj + j, 0, 0) ];
                interpolatePillar(&coord_h[ ch.index(Imin, J + 0, 0, 0) ],
                                  &coord_h[ ch.index(Imin, J + 1, 0, 0) ],
                                  tj, right);

                for (auto I = Imin; I <= Imax; ++I) {
                    // Set the left pillar to the previously computed right pillar
                    const auto* const left = right;
                    right = &coord_c[ cc.index((I + 1 - Imin)*si, (J - Jmin)*sj + j, 0, 0) ];

                    // Interpolates the right pillar for I + 1 in between for tj in between Jmin and Jmax
                    interpolatePillar(&coord_h[ ch.index(I + 1, J + 0, 0, 0) ],
                                      &coord_h[ ch.index(I + 1, J + 1, 0, 0) ],
                                      tj, right);

                    // Left and Right contain the pillars pillar for (I,j) and (I+1,) respectively.
                    for (auto i = 0*si + 1; i < si; ++i) {
                        const auto ti = static_cast<double>(i) / si;
                        const auto refinedPillar = cc.index((I - Imin)*si + i, (J - Jmin)*sj + j, 0, 0);

                        interpolatePillar(left, right, ti, &coord_c[refinedPillar]);
                    }
                }
            }
        }
