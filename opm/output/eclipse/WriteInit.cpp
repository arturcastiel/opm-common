                        ...
                        ::Opm::EclIO::OutputStream::Init&           initFile)
{
    ...
    // GLOBAL REGION ARRAYS (SATNUM, PVTNUM, EQLNUM, FIPNUM, ...)
    writeIntegerCellProperties(es, initFile);
    writeIntegerMaps(std::move(int_data), initFile);
    ...
    // Aquifers not yet supported for LGR
    if (es.aquifer().active()) {
        writeAquifers(es.aquifer(), grid, initFile);
    }

    // LGR NNC SECTION (for each LGR: LGR codeph, LGRHEADI/Q/D, TRANNNC, TRANGL, [LGRJOIN, TRANLL]) + LGRSGONE
    // TODO: implement TRANNNC, TRANGL, LGRJOIN, TRANLL inside writeLGRnnc.
    writeLGRnnc(es, grid, schedule, initFile, nnc_col);

    // LGR PROPERTY SECTION (for each LGR: LGR codeph, LGRHEADI/Q/D, INTEHEAD, LOGIHEAD, DOUBHEAD,
    //   PORV, DEPTH, TRANX/Y/Z, DX/Y/Z, PERMX/Y/Z, MULTX/Y/Z, PORO, TOPS, SATNUM, ...) + LGRSGONE
    writeLGRLocalProperties(es, grid, schedule, simProps, porv, units, initFile);

    // TABULAR DATA (TABDIMS, TAB, CON)
    writeTableData(es, units, initFile);

    // TODO: confirm correct position of writeSatFuncScaling from reference output
    writeSatFuncScaling(es, units, initFile);
